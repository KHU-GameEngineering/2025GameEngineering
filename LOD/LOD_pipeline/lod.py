...
def main():
    print("###################################################")
    print("###       3D Mesh Optimization Pipeline         ###")
    print("###################################################")
    
    # 1. Input 파일 설정
    INPUT_FILE = "instant_mesh_raw.obj" # 기본값
    current_dir = os.path.dirname(os.path.abspath(__file__))
    input_path = os.path.join(current_dir, "input", INPUT_FILE)
    output_dir = os.path.join(current_dir, "output_unified")
    
    if not os.path.exists(output_dir): os.makedirs(output_dir)
    if not os.path.exists(input_path):
        print(f"[오류] 'input' 폴더에 '{INPUT_FILE}' 파일이 없습니다.")
        return

    # 2. 사용자 입력 (Low vs High)
    while True:
        ...
        if choice_type == "1": # Low Poly
            ...
            if choice_action == "A":
                run_simple_clean(input_path, output_dir)
            elif choice_action == "B":
                run_academic_upsampling(input_path, output_dir)
            else:
                print("잘못된 입력입니다.")
            break

        elif choice_type == "2": # High Poly
            ...
            mode_map = {"L": "LOW", "M": "MID", "H": "HIGH", "A": "ALL"}
            if choice_lod in mode_map:
                run_smart_decimation(input_path, output_dir, mode_map[choice_lod])
            else:
                print("잘못된 입력입니다.")
            break
